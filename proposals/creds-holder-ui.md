...
* Display
* Inputs, sensors
* Outputs (display, speaker, vibration, LED)
* Human senses
* Human body
...
| Input method | |
| --- | --- |
| Rotary encoder/wheel | User will be able to perform two actions<br/>because push encoder is not stable and not available for wheel<br/>So it's only auxiliary input |
| Group of buttons | User will able to perform all needed action if use single push, long push, double push, two buttons simultaneous push<br/>But other actions will be unavailable |
...
| Touch LCD | <br/>User will be able to perform any actions<br/>And you will have a freedom to write any GUI interface, buttons and etc.<br/>But there will be requirement for minimal RAM, Arduino is not good enough and similar MCU, for NRF52840 you will always has a risk to reach limit<br/>And good UI development is complex |
| Position sensor (accelerometer + gyroscope) | <br/>User will able to perform actions:<br/>1. Tilt Backward (= Move Down)<br/>2. Tilt Forward (= Move Up)<br/>3. Tilt Left (= Move Left)<br/>4. Tilt Right (= Move Right)<br/>5. Turn upside down (= Lock)<br/>6. Knock (= Enter/Accept)<br/>7. Shake horizontally (= "Switch")<br/>8. Shake vertically (= ???)<br/>And device will be more compact<br/>And it'll be usable by one hand<br/>And it'll be cheap<br/>But device will have to use cable (or wireless interface) to connect to User equipment because if plugged to USB port directly then movements of device are not available<br/>And development complexity is unknown<br/>And usability in non ideal conditions is unknown |
| Microphone, knock detection | User will able to perform one action ("Enter/Accept" or "Backspace")<br/>And can help to stably detect knocking<br/>But not all required action will be available |
...
| Multi color LED | User will be able to receive auxiliary signals<br/>But as a result LED maybe used only together with another outputs |

Let's think what combinations of inputs and outputs will give the best results with minimum hardware schema influence.

### Ideal Results

* Element "X" of system without increasing system complexity and without negative effects perform communication with User without decreasing confidence and security.
* There is no inputs and outputs but Device can communicate with User

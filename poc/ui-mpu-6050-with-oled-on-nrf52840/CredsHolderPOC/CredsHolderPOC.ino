...
void loop() {
  ...
  if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer)) { // Get the Latest packet 
      ...
      // float tappingLevel = sqrt(aaReal.x*aaReal.x + aaReal.y*aaReal.y + aaReal.z*aaReal.z);
      // float tappingLevel = sqrt(aa.x*aa.x + aa.y*aa.y + aa.z*aa.z);
      // float tappingLevel = abs(aaReal.x) + abs(aaReal.y) + abs(aaReal.z);
      // float tappingLevel = abs(aa.x) + abs(aa.y) + abs(aa.z);
      // if (Serial) { Serial.println(tappingLevel); ++nLines; }
      // if (Serial) { 
